...
class Signaling {
  ...
  Signaling(this._url, this._secret, this._userName, this._iceServer, this._isWatching):
    _iceServers = {
      'iceServers': [
        {
          'url': 'stun:$_iceServer',
  ...
  final Map<String, dynamic> _Remoteconstraints = {
    'mandatory': {
  ...
  Future<dynamic> webRtcSessionLookup({String sessionId}) {
    final Map<String, dynamic> bodyMap = <String, dynamic>{'customSessionId': sessionId};
    final Map<String, dynamic> headersMap =
    <String, dynamic>{
      'Authorization': 'Basic ${base64Encode(utf8.encode('OPENVIDUAPP:$_secret'))}'
    ...
    return ApiClient().request<Map<String, dynamic>>(Config(
        uri: Uri.parse('https://$_url/api/sessions/$sessionId'),
        headers: headersMap,
        body: RequestBody.json(bodyMap),
        method: RequestMethod.get,
        responseType: ResponseBody.json()
    )).then((Map<String, dynamic> jsonResponse) {
      print('◤◢◤◢◤◢◤◢◤◢◤ Create WebRTC session get response: $jsonResponse ◤◢◤◢◤◢◤◢◤◢◤');
      session = jsonResponse['id'];
      return true;
    }).catchError((error) {
      //__FIX_IT_IF_ERROR_409
  ...
  Future<dynamic> createWebRtcToken({@required String sessionId, @required String role}){
